...
public class RelationServer implements IService {
    private ServerNetworkService netWork;
    private ServiceState state;
    private Data serviceData;
    private int listenPort;

    public RelationServer() throws Exception {
        init(null);

        int acceptorGroupCount = 1;
        int IOGroupCount = SystemConst.AVAILABLE_PROCESSORS;

        RelationServerResponseMng responseMng = new RelationServerResponseMng();
        ...
        builder.setListener(new NetworkListener(RelationServerSessionMng.getInstance(), responseMng));
    ...
    @Override
    public void update() {
        RelationServerSessionMng.getInstance().update();
    }
